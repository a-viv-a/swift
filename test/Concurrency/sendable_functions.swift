...
actor A {
  ...
  @Sendable func f() { // expected-warning{{actor-isolated synchronous instance method 'f()' cannot be marked as '@Sendable'; this is an error in the Swift 6 language mode}}
    state = true
  }
...
struct S<T> { // expected-note{{consider making generic parameter 'T' conform to the 'Sendable' protocol}}
  ...
  @Sendable func test() {} // expected-warning{{instance method of non-Sendable type 'S<T>' cannot be marked as '@Sendable'; this is an error in the Swift 6 language mode}}
}
...
@available(SwiftStdlib 5.1, *)
@MainActor @Sendable func globalActorFunc() { } // expected-warning{{main actor-isolated synchronous global function 'globalActorFunc()' cannot be marked as '@Sendable'; this is an error in the Swift 6 language mode}}
