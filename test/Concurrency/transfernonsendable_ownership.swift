...
func testConsumingUseAfterConsumeError(_ x: consuming Klass) async { // expected-error {{'x' used after consume}}
  await consumeTransferToMain(x) // expected-warning {{sending 'x' risks causing data races; this is an error in the Swift 6 language mode}}
...
actor ActorTestCase {
  ...
  borrowing func test2() async {
    await transferToMain(k) // expected-warning {{sending 'self.k' risks causing data races; this is an error in the Swift 6 language mode}}
