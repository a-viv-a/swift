...
actor CastTest<T, U : NonSendableProtocol, V : AnyObject, V2 : NonSendableKlassProtocol> { // expected-note 8{{}}
  ...
  func f() async -> NonSendableProtocol {
    guard let c = await cnx else { fatalError() } // expected-swift5-warning {{non-Sendable type 'NonSendableKlass?' of property 'cnx' cannot exit main actor-isolated context; this is an error in the Swift 6 language mode}}
  ...
  func f2() async -> NonSendableProtocol {
    guard let c = await cnx2 else { fatalError() } // expected-swift5-warning {{non-Sendable type 'T?' of property 'cnx2' cannot exit main actor-isolated context; this is an error in the Swift 6 language mode}}
  ...
  func f3() async -> NonSendableProtocol {
    guard let c = await cnx3 else { fatalError() } // expected-swift5-warning {{non-Sendable type 'U?' of property 'cnx3' cannot exit main actor-isolated context; this is an error in the Swift 6 language mode}}
  ...
  func g() async -> NonSendableProtocol {
    guard let c = await cnx else { fatalError() } // expected-swift5-warning {{non-Sendable type 'NonSendableKlass?' of property 'cnx' cannot exit main actor-isolated context; this is an error in the Swift 6 language mode}}
    // expected-swift6-error @-1 {{non-Sendable type 'NonSendableKlass?' of property 'cnx' cannot exit main actor-isolated context}}
    let q = c as! NonSendableProtocol // expected-swift5-warning {{assigning main actor-isolated 'c' to 'self'-isolated 'q' risks causing data races; this is an error in the Swift 6 language mode}}
  ...
  func h() async -> NonSendableKlassProtocol {
    guard let c = await cnx else { fatalError() } // expected-swift5-warning {{non-Sendable type 'NonSendableKlass?' of property 'cnx' cannot exit main actor-isolated context; this is an error in the Swift 6 language mode}}
    // expected-swift6-error @-1 {{non-Sendable type 'NonSendableKlass?' of property 'cnx' cannot exit main actor-isolated context}}
    guard let q = c as? NonSendableKlassProtocol else { fatalError() } // expected-swift5-warning {{casting main actor-isolated 'c' to 'self'-isolated 'any NonSendableKlassProtocol' risks causing data races; this is an error in the Swift 6 language mode}}
    return q
  }

  func h2() async -> NonSendableKlassProtocol {
    guard let c = await cnx4 else { fatalError() } // expected-swift5-warning {{non-Sendable type 'V?' of property 'cnx4' cannot exit main actor-isolated context; this is an error in the Swift 6 language mode}}
  ...
  func h3() async -> NonSendableKlassProtocol {
    guard let c = await cnx5 else { fatalError() } // expected-swift5-warning {{non-Sendable type 'V2?' of property 'cnx5' cannot exit main actor-isolated context; this is an error in the Swift 6 language mode}}
  ...
  func k() async -> NonSendableKlassProtocol {
    guard let c = await cnx else { fatalError() } // expected-swift5-warning {{non-Sendable type 'NonSendableKlass?' of property 'cnx' cannot exit main actor-isolated context; this is an error in the Swift 6 language mode}}
    // expected-swift6-error @-1 {{non-Sendable type 'NonSendableKlass?' of property 'cnx' cannot exit main actor-isolated context}}
    let q = c as! NonSendableKlassProtocol // expected-swift5-warning {{assigning main actor-isolated 'c' to 'self'-isolated 'q' risks causing data races; this is an error in the Swift 6 language mode}}
    // expected-swift5-note @-1 {{'c' could become accessible to 'self'-isolated code despite remaining accessible to main actor-isolated code}}
    // expected-swift5-warning @-2 {{casting main actor-isolated 'c' to 'self'-isolated 'any NonSendableKlassProtocol' risks causing data races; this is an error in the Swift 6 language mode}}
    return q
  }
